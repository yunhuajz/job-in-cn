...
import { crawlerConfigSchema, experienceNames, type ExperienceOption } from "@/lib/local/preferences";
...
export default function LocalJobs() {
  ...
  function openOriginalJob(job: JobRow) {
    if (job.progress === 'draft') void updateProgressFor([job.id], 'applied');
  }
  ...
  async function scoreJobs(ids: string[]) {
    ...
    try {
      ...
      for (let i = 0; i < ids.length; i += CHUNK_SIZE) {
        ...
        if (ids.length > 1) {
          setNotice(`正在通过 AI 模型评分中 (${i + 1}-${Math.min(i + CHUNK_SIZE, ids.length)} / 共 ${ids.length} 个)…`);
        } else {
          setNotice('正在通过 AI 模型评分中，请稍候…');
        }
        const response = await fetch('/api/local/score', {
          method: 'POST',
        ...
        totalScored += body.scored ?? 0;
        totalFailed += body.failed ?? 0;
      }
      setNotice(`已完成 ${totalScored} 个岗位的 LLM 评分${totalFailed ? `，${totalFailed} 个失败` : ''}`);
  ...
  function applyFilters(next = filters) {
    try {
      const sanitized = {
        ...next,
        weekend: (next.weekend === 'yes' || next.weekend === 'no' ? next.weekend : 'any') as 'any' | 'yes' | 'no',
      };
  ...
  if (filters.experience && filters.experience !== 'any') {
    activeChips.push({ id: 'experience', label: `年限: ${experienceNames[filters.experience as ExperienceOption] || filters.experience}`, onRemove: () => setAndApply({ experience: 'any' }) });
  }
  if (filters.weekend && filters.weekend !== 'any') {
    const weekendMap: Record<string, string> = {
      yes: '双休: 仅明确双休',
      yes_or_unknown: '双休: 包含待确认',
      no: '双休: 非双休/单休',
      unknown: '双休: 仅待确认',
    };
    activeChips.push({ id: 'weekend', label: weekendMap[filters.weekend] || `双休: ${filters.weekend}`, onRemove: () => setAndApply({ weekend: 'any' }) });
  }
  if (filters.score && filters.score !== 'all') {
    const scoreMap: Record<string, string> = {
      '4_plus': '评分: ≥ 4.0 分 (高匹配)',
      '3.5_plus': '评分: ≥ 3.5 分 (良好)',
      '3_plus': '评分: ≥ 3.0 分 (及格)',
      'under_3': '评分: < 3.0 分',
      'scored': '评分: 已评分',
      'unscored': '评分: 未评分',
    };
    activeChips.push({ id: 'score', label: scoreMap[filters.score] || `评分: ${filters.score}`, onRemove: () => setAndApply({ score: 'all' }) });
  }
  if (filters.progress) {
    activeChips.push({ id: 'progress', label: `进度: ${progressNames[filters.progress as Progress] || filters.progress}`, onRemove: () => setAndApply({ progress: '' }) });
  }
  if (filters.source) {
    activeChips.push({ id: 'source', label: `来源: ${filters.source}`, onRemove: () => setAndApply({ source: '' }) });
  }
  if (filters.from || filters.to) {
    activeChips.push({ id: 'date', label: `采集日期: ${filters.from || '起'} ~ ${filters.to || '止'}`, onRemove: () => setAndApply({ from: '', to: '' }) });
  }
  ...
  return <div className="mx-auto max-w-7xl space-y-6">
    ...
    <form className="space-y-5 rounded-2xl border bg-card p-5 shadow-sm sm:p-6" onSubmit={(event) => { event.preventDefault(); applyFilters(); }}>
      ...
      <div className="mt-5 grid gap-4 sm:grid-cols-3 lg:grid-cols-4">
        <label className="text-sm font-medium">评分状态<select className={fieldClass} value={filters.score} onChange={(e) => setFilters({ ...filters, score: e.target.value })}><option value="all">全部评分</option><option value="4_plus">4.0 分及以上 (≥80)</option><option value="3.5_plus">3.5 分及以上 (≥70)</option><option value="3_plus">3.0 分及以上 (≥60)</option><option value="under_3">3.0 分以下</option><option value="scored">已评分</option><option value="unscored">未评分</option></select></label>
      ...
      <div className="mt-5 border-t pt-5"><PreferenceFields value={{ ...filters, weekend: (filters.weekend === 'yes' || filters.weekend === 'no' ? filters.weekend : 'any') as 'any' | 'yes' | 'no' }} onChange={(value) => setFilters({ ...filters, ...value })} /></div>
      </details>
      <div className="flex justify-end gap-3">{activeChips.length > 0 && <Button type="button" variant="outline" onClick={clearAllFilters}>重置条件</Button>}<Button type="submit" disabled={loading}>应用筛选</Button></div>
    </form>
    {activeChips.length > 0 && (
      <div className="flex flex-wrap items-center gap-2 rounded-2xl border bg-card p-4 shadow-sm text-sm">
        <span className="font-semibold text-primary">已生效组合条件 ({activeChips.length})：</span>
    ...
    <section className="overflow-hidden rounded-2xl border bg-card shadow-sm" aria-busy={loading}>
      <div className="flex flex-wrap items-center justify-between gap-3 border-b p-4 text-sm">
        <div className="flex items-center gap-2">
          <span className="font-semibold">{loading ? '正在读取…' : `共 ${data?.total ?? 0} 个岗位`}</span>
          {activeChips.length > 0 && <span className="rounded-full bg-primary/10 px-2.5 py-0.5 text-xs font-medium text-primary">多条件组合筛选中</span>}
        </div>
      ...
      <div className="overflow-x-auto"><table className="w-full text-left text-sm"><thead className="bg-muted/40 text-muted-foreground"><tr><th className="px-4 py-3" aria-label="选择岗位" /><th className="whitespace-nowrap px-4 py-3 font-medium"><button type="button" onClick={() => sortBy('title_asc', 'title_desc')}>岗位 / 公司{sortMark('title_asc', 'title_desc')}</button></th><th className="whitespace-nowrap px-4 py-3 font-medium"><button type="button" onClick={() => sortBy('location_asc', 'location_desc')}>地点{sortMark('location_asc', 'location_desc')}</button></th><th className="whitespace-nowrap px-4 py-3 font-medium"><button type="button" onClick={() => sortBy('salary_asc', 'salary_desc')}>月薪{sortMark('salary_asc', 'salary_desc')}</button></th><th className="whitespace-nowrap px-4 py-3 font-medium">双休</th><th className="whitespace-nowrap px-4 py-3 font-medium"><button type="button" onClick={() => sortBy('score_asc', 'score_desc')}>评分{sortMark('score_asc', 'score_desc')}</button></th><th className="whitespace-nowrap px-4 py-3 font-medium">进度</th><th className="whitespace-nowrap px-4 py-3 font-medium"><button type="button" onClick={() => sortBy('collected_asc', 'collected_desc')}>最近采集{sortMark('collected_asc', 'collected_desc')}</button></th><th className="whitespace-nowrap px-4 py-3 font-medium"><button type="button" onClick={() => sortBy('source_asc', 'source_desc')}>来源{sortMark('source_asc', 'source_desc')}</button></th></tr>
        <tr className="border-t border-border/60">
          <th />
          <th className="px-3 pb-3"><input aria-label="按岗位或公司筛选" className="h-8 min-w-40 rounded-md border bg-background px-2 text-xs" value={filters.q} placeholder="搜索岗位或公司" onChange={(event) => setFilters({ ...filters, q: event.target.value })} onBlur={() => applyFilters()} onKeyDown={(event) => { if (event.key === 'Enter') { event.preventDefault(); applyFilters(); } }} /></th>
          <th className="px-3 pb-3"><input aria-label="按地点筛选" className="h-8 w-32 rounded-md border bg-background px-2 text-xs" value={filters.city} placeholder="地点 (如: 北京 上海)" onChange={(event) => setFilters({ ...filters, city: event.target.value })} onBlur={() => applyFilters()} onKeyDown={(event) => { if (event.key === 'Enter') { event.preventDefault(); applyFilters(); } }} /></th>
          <th className="px-3 pb-3 text-xs">点击排序</th>
          <th className="px-3 pb-3"><select aria-label="按双休筛选" className="h-8 w-28 rounded-md border bg-background px-2 text-xs font-medium" value={filters.weekend} onChange={(event) => setAndApply({ weekend: event.target.value as any })}><option value="any">全部双休</option><option value="yes">仅明确双休</option><option value="yes_or_unknown">双休或待确认</option><option value="no">非双休/单休</option><option value="unknown">仅待确认</option></select></th>
          <th className="px-3 pb-3"><select aria-label="按评分筛选" className="h-8 w-32 rounded-md border bg-background px-2 text-xs font-medium" value={filters.score} onChange={(event) => setAndApply({ score: event.target.value })}><option value="all">全部评分</option><option value="4_plus">⭐ 4.0及以上 (≥80)</option><option value="3.5_plus">⭐ 3.5及以上 (≥70)</option><option value="3_plus">⭐ 3.0及以上 (≥60)</option><option value="under_3">3.0以下 (&lt;60)</option><option value="unscored">未评分</option><option value="scored">已评分 (全部)</option></select></th>
          <th className="px-3 pb-3"><select aria-label="按进度筛选" className="h-8 w-28 rounded-md border bg-background px-2 text-xs font-medium" value={filters.progress} onChange={(event) => setAndApply({ progress: event.target.value })}><option value="">全部进度</option>{Object.entries(progressNames).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></th>
          <th className="px-3 pb-3 text-xs">点击排序</th>
        ...
        <div className="p-12 text-center text-muted-foreground space-y-3">
          ...
          {activeChips.length > 0 ? (
            ...
            <Link href="/dashboard/crawler">
              <Button size="sm">前往爬虫页采集新岗位</Button>
            </Link>
          )}
        </div>
      )}
