...
export interface EvaluateJobsInput {
  ids: string[];
...
export async function evaluateJobsForUser(
  input: EvaluateJobsInput,
  userId: string,
  ...
  const settings: UserSettingsData = row
    ? { ...defaultUserSettings, ...JSON.parse(row.settings) }
    : defaultUserSettings;

  const activeProfile =
    settings.aiProfiles?.find((p) => p.isActive) || settings.aiProfiles?.[0];
  ...
  if (!targetModel) {
    throw new Error("请先在“AI 设置”中选择评分模型");
  }

  const resume = await getDefaultResumeForUser(userId);
  if (!resume) {
    throw new Error("请先在个人资料中设置默认简历");
  }
  ...
  if (!prepared.success) {
    if (
      prepared.error.code === "TOO_SHORT" ||
      prepared.error.code === "NO_CONTENT"
    ) {
  ...
  const model = await getModel(
    settings.ai.provider as ProviderType,
    targetModel,
    userId,
    { baseURL: targetBaseURL, protocol: targetProtocol },
  );

  const jobs = await prisma.job.findMany({
    where: { userId, id: { in: [...new Set(input.ids)] } },
    select: {
      id: true,
      description: true,
      salaryRange: true,
  ...
  const runGenerate = input.generateFn ?? generateText;
  ...
  for (const job of jobs) {
    try {
      ...
      const result = await runGenerate({
        ...
        prompt: buildJobMatchPrompt(prepared.data.normalizedText, jobText),
        temperature: 0.3,
      });

      const parsed = parseJobMatch(result.text);
      if (!parsed.scores) {
        throw new Error("模型没有返回可识别的评分");
      }
      ...
      failures.push(
        `${job.JobTitle.label}：${error instanceof Error ? error.message : "评分失败"}`,
      );
    }
  }

  return { scored, failed: failures.length, failures };
}
