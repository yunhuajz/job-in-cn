...
export async function ingestJob(
  ...
  if (!allowDuplicate) {
    ...
    if (duplicate) {
      await prisma.jobCollection.create({ data: { jobId: duplicate.id, collectedAt } });
      return {
        created: false,
        jobId: duplicate.id,
  ...
  const tagIds = resolvedTagsResult.resolved.map((t) => t.id);

  // Inlined Job creation (eliminates shallow createJobRecord passthrough)
  ...
  await prisma.jobCollection.create({ data: { jobId: job.id, collectedAt } });
