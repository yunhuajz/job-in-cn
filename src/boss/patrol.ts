...
function runScript(scriptPath: string, timeoutMs: number): Promise<RunResult> {
  return new Promise((resolveRun) => {
    const child = spawn(
      process.execPath,
      ['node_modules/tsx/dist/cli.mjs', scriptPath],
      { cwd: resolve('.') },
    );
    let output = '';
...
async function main(): Promise<void> {
  ...
  while (Date.now() < deadline) {
    cycle += 1;
    log(`--- 第 ${cycle} 轮开始 ---`);
    const harvest = await runScript('src/boss/harvest.ts', HARVEST_TIMEOUT_MS);
    if (harvest.timedOut) {
      log('采集子进程超时已终止,本轮直接进入评分');
    }
    if (/异常行为|风控/.test(harvest.output)) {
      log('检测到账户风控熔断:停止今日采集,巡逻退出。请手动验证后再启动。');
      break;
    }
