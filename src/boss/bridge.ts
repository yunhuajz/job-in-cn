...
import { resolveBossCityCode, BOSS_CITY_CODES } from './city-codes.js';
export { resolveBossCityCode, BOSS_CITY_CODES };
...
export async function bossSearch(
  ...
  if (options.city) {
    const code = resolveBossCityCode(options.city);
    args.push('--city', code ?? options.city);
  }
