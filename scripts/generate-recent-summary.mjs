...
const outputPath = 'D:/AJS/job-for-claude/data/岗位汇总-昨晚至今-2026-09-01.html';
const from = Date.parse('2026-08-31T18:00:00+08:00');
const to = Date.now();

const sql = `SELECT
  j.jobUrl AS url,
  j.matchScore AS score,
  j.salaryRange AS salary,
  j.createdAt AS createdAt,
  t.value AS title,
  c.value AS company,
  l.value AS location,
  j.jobSourceId AS sourceId,
...
function summaryFor(job) {
  try {
    const match = JSON.parse(job.matchData ?? '{}');
    return match.summary || '暂无评分摘要';
  } catch {
    return '暂无评分摘要';
  }
}
...
function sourceFor(url) {
  ...
  if (url.includes('51job.com')) return '前程无忧51job';
...
.title{font-size:16px;font-weight:600;margin-bottom:4px}.meta{font-size:13px;color:#555;margin-bottom:4px}.meta b{color:#dc2626;font-weight:600}
...
<body>
<header>
<h1>岗位汇总（共 ${jobs.length} 个）</h1>
<div class="sub">时间范围：2026/8/31 18:00 至 2026/9/1 · 生成时间：${generatedAt} · 点击岗位卡片跳转原始招聘页</div>
<input id="q" type="search" placeholder="搜索岗位 / 公司 / 城市 / 来源">
</header>
<main id="list">${cards.join('\n')}</main>
<script>
const input=document.getElementById('q');
const cards=[...document.querySelectorAll('.card')];
input.addEventListener('input',()=>{const q=input.value.trim().toLowerCase();cards.forEach(c=>c.classList.toggle('hidden',Boolean(q)&&!c.dataset.search.toLowerCase().includes(q)));});
</script>
</body>
</html>
`;
...
console.log(JSON.stringify({ outputPath, count: jobs.length, from, to }));
